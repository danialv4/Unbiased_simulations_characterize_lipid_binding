...
martinize2 -f step1_pdbreader_his.pdb -x prot_cg_fc1000_eu8.pdb -o topol.top -ff martini3001 -elastic -ef 1000 -eu 0.8 -nt -scfix -cys auto -dssp /usr/bin/dssp 

#CHOOSE -merge <chainID> for multimers

#Inserting the CG protein in a cubic box with 2 nm of distance
gmx editconf -f prot_cg_fc1000_eu8.pdb  -c -d 2.0 -bt cubic -o prot_cg_newbox.gro

cp ./common/cg.top .

#First minimization in vaccuum
gmx grompp -f ./common/min-vac.mdp -p cg.top -c prot_cg_newbox.gro -o min-vac.tpr
...
do
	...
	#TPR for production is generated
        gmx grompp -f ./common/martini_v2.x_new-rf.mdp -c  npt_eq_${lipid}_seed${replica}.gro -t npt_eq_${lipid}_seed${replica}.cpt -n index_${lipid}_seed${replica}.ndx -p ${lipid}_seed${replica}.top -o md_${lipid}_seed${replica}.tpr -maxwarn 1
        done
done
